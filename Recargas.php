...
class Recargas {
    ...
    public function mostrarRecargas(){
        ...
        $sql = "SELECT descricao, preco, nserie FROM recargas";
        $query = $this->conexao->query($sql);
        $mostrar = $query->fetchAll();
        
        return $mostrar;
    }
    
    public function recargaSelecionada($nSerie){
        
        $this->conexao = new Conexao();
        $sql = "SELECT * FROM recargas WHERE nserie = ".$nSerie;
        $query = $this->conexao->query($sql);
        $mostrar = $query->fetchAll();
